...
| CPU-сон | нет | нет |
| Стартовый звук | нет | **Имперский марш** 🎵 |
| Частота CPU | 160 МГц всегда | **160→80 МГц** через 30 сек |
| Двойной клик → радио | нет | **F/BW/SF/CR** ✅ |
...
**Динамическая частота CPU:** при старте 160 МГц, через 30 сек автоснижение до 80 МГц (экономия ~30-40% потребления ядра). Кнопка возвращает 160 МГц на время бодрствования экрана. BLE и LoRa на 80 МГц работают стабильно.

**Имперский марш:** при подаче питания buzzer проигрывает Imperial March (Star Wars) — 18 нот, ~5.5 сек. Мелодия **неблокирующая** (играет параллельно с загрузкой).

**Двойной клик:** при бодрствующем экране два быстрых нажатия кнопки (<400 мс) показывают экран с настройками: частота (F), bandwidth (BW), spreading factor (SF), coding rate (CR). Автовозврат через 5 сек.

**Изменения в исходном коде:**
- `UITask.cpp` — сообщения не будят дисплей в режиме `POWER_SAVING`, таймер кнопки 5 сек, динамическая частота CPU, двойной клик с радио-экраном
- `UITask.h` — поля для двойного клика и экрана настроек
- `ESP32Board.h` — добавлена поддержка light sleep для ESP32-C6, метод `sleepMs()`
- `main.cpp` — Imperial March неблокирующий, `tickMelody()` в главном цикле
- `platformio.ini` — новый env `m5stack_unit_c6l_companion_radio_ble_lp` с флагами LP
