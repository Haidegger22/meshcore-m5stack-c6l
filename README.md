...
⚠️ **Важно:** обычное удержание RESET (3 сек до красного светодиода) **не работает** на Raspberry Pi / Linux — устройство определяется, но сразу отключается от USB. Используйте метод с BOOT:

1. Отключите USB-C от C6L
2. Зажмите кнопку **BOOT** (боковая) и держите
3. Подключите USB-C (не отпуская BOOT)
4. Отпустите BOOT
5. Устройство появится как `/dev/ttyACM0`
